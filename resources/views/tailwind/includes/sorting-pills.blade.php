        ...
        <div class="md:mb-4 px-6 py-2 md:p-0">
            ...
            @foreach($sorts as $col => $dir)
                <span
                    ...
                    class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium leading-4 {{ $this->customThemeColor ? "bg-{$this->customThemeColor}-100 text-{$this->customThemeColor}-800 dark:bg-{$this->customThemeColor}-200 dark:text-{$this->customThemeColor}-900" : 'bg-indigo-100 text-indigo-800 dark:bg-indigo-200 dark:text-indigo-900' }} capitalize"
                >
