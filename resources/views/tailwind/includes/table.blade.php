<x-livewire-tables::table wire:sortable="{{ $reordering ? $reorderingMethod : '' }}"
    ...
    <x-slot name="head">
        ...
        <x-livewire-tables::table.heading />
        @else
        <x-livewire-tables::table.heading :sortingEnabled="$sortingEnabled"
            ...
            <x-livewire-tables::table.cell>
                <div class="inline-flex rounded-md shadow-sm">
                    <input wire:model="selected"
                           ...
                           type="checkbox"
                           class="rounded border-gray-300 shadow-sm transition duration-150 ease-in-out {{ $this->customThemeColor ? "text-{$this->customThemeColor}-600 focus:border-{$this->customThemeColor}-300 focus:ring-{$this->customThemeColor}-200" : 'text-{$this->customThemeColor}-600 focus:border-indigo-300 focus:ring-indigo-200' }} focus:ringfocus:ring-opacity-50 dark:bg-gray-900 dark:text-white dark:border-gray-600 dark:hover:bg-gray-600 dark:focus:bg-gray-600" />
                </div>
            </x-livewire-tables::table.cell>
            @endif
    ...
    <x-slot name="foot">
        <x-livewire-tables::table.row wire:loading.class.delay="opacity-50 dark:bg-gray-900 dark:opacity-60"
            ...
            @if ($column->hasFooter())
            <x-livewire-tables::table.footer :class="method_exists($this, 'setFooterDataClass') ? $this->setFooterDataClass($column, $rows) : ''"
