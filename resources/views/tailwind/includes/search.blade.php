@if ($showSearch)
    <div class="flex rounded-md shadow-sm">
        <input
            ...
            type="text"
            class="block w-full border-gray-300 rounded-md shadow-sm transition duration-150 ease-in-out sm:text-sm sm:leading-5 dark:bg-gray-700 dark:text-white dark:border-gray-600 @if (isset($filters['search']) && strlen($filters['search'])) rounded-none rounded-l-md focus:ring-0 focus:border-gray-300 @else {{ $this->customThemeColor ? "focus:border-{$this->customThemeColor}-300 focus:ring-{$this->customThemeColor}-200" : 'focus:border-indigo-300 focus:ring-indigo-200' }} focus:ring  focus:ring-opacity-50 rounded-md @endif"
        />
