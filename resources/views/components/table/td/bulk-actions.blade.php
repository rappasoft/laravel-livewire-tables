...
@if ($component->bulkActionsAreEnabled() && $component->hasBulkActions())
    <x-livewire-tables::table.td.plain  wire:key="{{ $tableName }}-tbody-tr-{{ $row->{$this->getPrimaryKey()} }}-hidden" x-show="currentlyReorderingStatus" />

    <x-livewire-tables::table.td.plain  wire:key="{{ $tableName }}-tbody-tr-{{ $row->{$this->getPrimaryKey()} }}-shown" x-show="!currentlyReorderingStatus">
        <div @class([
