...
@if ($component->bulkActionsAreEnabled() && $component->hasBulkActions())
    ...
    <x-livewire-tables::table.td.plain wire:key="{{ $tableName }}-tbody-td-bulkactionstd-{{ $row->{$this->getPrimaryKey()} }}" :displayMinimisedOnReorder="true"  >
        <div
            @class([
            'inline-flex rounded-md shadow-sm' => $theme === 'tailwind',
            'form-check' => $theme === 'bootstrap-5',
        ])>
            <input x-show="!currentlyReorderingStatus"
