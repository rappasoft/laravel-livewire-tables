...
@if ($component->isTailwind())
    <th scope="col" {{
        $attributes->merge($customAttributes)
            ->class(['px-6 py-3 '. config('livewire-tables.dir' , 'ltr') == 'ltr'? 'text-left' : 'text-right mr-4' .' text-xs font-medium whitespace-nowrap text-gray-500 uppercase tracking-wider dark:bg-gray-800 dark:text-gray-400' => $customAttributes['default'] ?? true])
                ...
                <button
                    ...
                    {{
                        $attributes->merge($customSortButtonAttributes)
                            ->class(['flex items-center space-x-1 '. config('livewire-tables.dir' , 'ltr' ) =='ltr'? 'text-left' : 'text-right mr-4' .' text-xs leading-4 font-medium text-gray-500 uppercase tracking-wider group focus:outline-none dark:text-gray-400' => $customSortButtonAttributes['default'] ?? true])
