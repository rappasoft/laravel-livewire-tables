<div>
    ...
    @if ($isBootstrap)
        @php($allSelected = count($this->getAppliedFilterWithValue($filter->getKey()) ?? []) === count($filter->getOptions()))
        <div class="lwt-ms" role="group">
            <button
                type="button"
                wire:click="selectAllFilterOptions('{{ $filter->getKey() }}')"
                @class(['lwt-ms__chip', 'lwt-ms__chip--all', 'is-active' => $allSelected])
            ...
            @foreach($filter->getOptions() as $key => $value)
                <div class="lwt-ms__option" wire:key="{{ $tableName }}-filter-{{ $filter->getKey() }}-multiselect-{{ $key }}{{ $filter->hasCustomPosition() ? '-'.$filter->getCustomPosition() : null }}">
                    <input
                        ...
                        wire:key="{{ $tableName }}-filter-{{ $filter->getKey() }}-input-{{ $loop->index }}{{ $filter->hasCustomPosition() ? '-'.$filter->getCustomPosition() : null }}"
                        type="checkbox"
                        class="lwt-ms__input"
                        value="{{ $key }}"
                    />
